...
with $\mu>1$ and $\lambda>1$, a strong selective pressure can occur in 
...
which bought the book. The dataset contains a subsample of 1,300~customers with 10~exploratory variables (see Table~\ref{tab:BBBClubDescription}) for building a predictive model of customer choice. 
...
that, the default settings of the algorithms are used. For the assessment
